...
One species found in Pindanland is the Bar-tailed Godwit, *Limosa lapponica* (left). Once a year, it migrates to Australia for food across the Pacific for up to [11,000 kilometers](https://doi.org/10.1093/condor/107.1.1) non-stop. This is considered one of the longest continuous journeys by any bird in the world!
...
There are also some extra ways you can increase the readability of your ridgeline plot. For example, we can make trends clearer by ordering species by mean or averages, which will help place birds with greater abundance at the end of the year towards the top, and birds with greater abundance at the beginning of the year at the bottom. We can also adjust the colours and axis labels. We chose the `theme_pilot` from the pilot package as it uses a colour-blind friendly palette. We also adjust the the smoothness of our ridges to account for more fine-scale variation if that is something of interest using `bandwidth`. 
...
shorebirds_filtered |>
  ...
  scale_x_continuous(
    breaks = c(1, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330),  # set numbers for labels every two months including December
    labels = c("Jan", "Feb", "Mar", "Apr", "May", "June", "July", "Aug", "Sept", "Oct", "Nov", "Dec"),  # add labels
    expand = c(0,0)) + 
  labs(x = "Month") +
  ggridges::geom_density_ridges(color = NA,
    bandwidth = 16, # this controls the smoothness of the curve.
    scale = 3,          # adjust the scale to widen the ridges
    height = 0.05,      # adjust the height to increase space between ridges
    alpha = .8, # adjust transpareny level of ridges 
    rel_min_height = 0.02) +
...
Keep in mind that ridgeline plots might need adjusting to visualise your data clearly, depending on your purpose of visualisation. In the first example, the ridgelines are presented as smooth curves that prevent you from seeing substantial individual points or breaks and variation in the data. Smooth ridgeline graphs are useful to present broader trends, but not necessarily to assess detailed or finer scale components of your data. 
