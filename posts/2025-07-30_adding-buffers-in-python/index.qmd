...
How this affects our ability to know the true location of a species in an area is illustrated in the diagram below. The *true* location of the point will snap to a grid, the distance of which depends on its degree of generalisation. Whether the point snaps up or down will depend on which direction the point was rounded[^snapping]. What this means is that it's possible that a given data point falls within any of the following three scenarios:
...
Let's consider how different buffer sizes impact our ability to detect sensitive species.  Below, we've created six buffers of increasing size around Mid-Western, with a grid of 10 km blocks drawn over our map. These generalised points are. The stuttering frog observations are *just* outside of our area; without a buffer this frog species would go undetected, but with a 30km buffer we would detect several observations. Which should we choose?

<!-- The new strategy here is instead of circles around our points, plot the points, then place a 10 km grid over the top. Ideally, the points will align to the grid like-->
