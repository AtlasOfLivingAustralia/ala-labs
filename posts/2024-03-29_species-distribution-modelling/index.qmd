...
The process of fitting and choosing the best model is iterative and, often, predictions across different modelling methods are stacked into one workflow. Here, we walk through the steps of building a SDM for the laughing kookaburra in a small area of New South Wales, Australia, using the [tidymodels](https://www.tidymodels.org/) and [tidysdm](https://evolecolgroup.github.io/tidysdm/). tidymodels is a collection of packages for modelling using [tidyverse principles](https://tidyverse.tidyverse.org/articles/manifesto.html), and tidysdm implements SDMs using the tidymodels framework.

<!-- I'm confused. Is it typical for species distribution models to use parts of many types of models (ie, random forest, MaxEnt, GLM, etc)? I know choosing predictors and weights is iterative, but I rarely see multi-model implementations to create species distribution in the literature. I figured this was new-ish thanks to tools like tidymodels and therefore might be useful. - DK-->

<!-- Yeah I'm also a little lost at this part, but that could just be a result of my limited knowledge on the methods behind selecting appropriate SDM's. I think you just need to be a little clearer on what exactly makes the process iterative before we dive in to the evidently lengthy process. Even flagging that this will become clear as we go through the steps?. Having a lengthy article isn't necessarily bad to me and I don't think you should cut any more, I'd rather have everything I need to know in front of me vs. searching rigorously through the web. You've done well to cut some already though and I think it is looking good - OT --> 

<!-- I think we're all confused now. The idea of ensemble modelling has been around for a good 15 years or so, and biomod2 is probably the most popular choice for people using R. There are (I think) other tools in R that do this as well, but are not as popular. It is also possible to assemble ensembles without tools like this but I think this is also less common. I'm happy for you to remove the reference to iteration if it's confusing. I was referring to cross-validation, which is iterative in the sense that subsets of the data are used iteratively for training and fitting or testing. - SB --> 
...
[Tidymodels is optimised for machine learning, but it isn't necessarily "better" than other packages like {dismo}, {lme4}, or packages for bayesian models like {tidybayes} or {brms}.]{style="font-size:.7rem;"}
...
library(galah)
library(tidyverse)
library(tidymodels) # automatically loaded with tidysdm I think [I think I want it there anyway]
...
For many of the later steps we'll need the coordinates formatted as a spatial object (i.e., `geometry`). So, let's convert our occurrence data to a spatial object (`sf`) defined by the longitude and latitude coordinates, and set the Coordinate Reference System (CRS) to EPSG:4326[^1] (WGS84).

[^1]: ALA data is projected using [CRS EPSG:4326](https://epsg.io/4326) (the same one used by Google Earth).
...
So, we can **thin** our data so that there is only one observation per cell of our raster[^2]. This step reduces spatial bias and lowers the risk that autocorrelation affects final predictions of our model.

[^2]: It's also possible to thin data by distance rather than cell size using [`tidysdm::thin_by_dist()`](https://evolecolgroup.github.io/tidysdm/reference/thin_by_dist.html).
...
Now that we have our presence and pseudo-absence points, the third step is to extract the environmental data for each point location in `aus` and bind the resulting values to our points data in `kookaburras_psuedoabs`. The result is a `tibble` with our points, their class, and the specific values of all 24 BioClim variables.
...
The fourth and final step is to choose predictor variables for our model. These are variables we think explain variation in our outcome (i.e., the probability a kookaburra could live in a given location). When choosing predictor variables, it is good practice to use theory and previous research to inform what variables you choose as predictors[^4].

[^4]: Keep in mind that the strength of variables depends on the scale of your prediction. If you wish to make predictions at a broad-scale, variables like temperature and rainfall will likely be strong predictors, whereas if you wish to make predictions at a fine-scale, variables like food scarcity and competition might be stronger predictors for your outcome.

:::{.callout-note collapse="true"}

## Choosing variables and avoiding multicollinearity

In species distribution models, *multicollinearity*---high correlation between several independent variables in a model---can have unintended effects that bias predictions[^5]. Data science tools can also help refine your predictor variable choices, too, including [some functions in tidysdm](https://evolecolgroup.github.io/tidysdm/articles/a0_tidysdm_overview.html#thinning-step) that we used below.

[^5]: A model can only use the information it is provided to make inferences about the world. If multiple variables in a model correlate, the model can place too much weight on those values to determine the outcome! The model isn't aware of the many other environmental variables that affect the real world outcome.
...
Dax: I think this was the fast way I wrote that I only used the tidysdm tools

SB: I think I found the phrasing confusing because I didn't know what other techniques we could have used. I think the text within parentheses could either be omitted entirely or, if you think it loses too much, perhaps say something like "based on tests for collinearity (or whatever the tidysdm tools were)"?
...
Dax: Well I don't really know how suitable they are, other than knowing they seem to vary between presences and absences and they are collinear with each other. These steps were removed after a previous conversation we had because I agreed they might be overkill given how long the article already is.
...
-------
I agree that at the moment this section is a bit unclear, but I don't agree with ignoring these steps completely. I feel like variable choice is one of the most crucial parts of SDM model building (simply because I imagine this is where people might face scrutiny if the model output is inaccurate or unexpected). I have tried to streamline this process and placed it in a tab. People who care can read about it, but people could also skip over it completely. I don't know if this solves the order issue you raised earlier, but 

SB: I think the current version addresses the issues around clarity I raised before, so I'm happy with how it reads here. 
-->
```
-   **BIO4**: Temperature Seasonality[^6]
-   **BIO12**: Annual Precipitation

[^6]: Measured as the standard deviation of the mean monthly temperature
...
Next let's make our model's "recipe". This is the tidymodels term for any pre-processing steps that happen to our data before adding them to a model. A recipe includes our model formula, and any transformations or standardisations we might wish to do[^7].

[^7]: For example, log transformation, centring scales, setting dummy variables

In our case, let's define that our model's outcome variable[^8] is the `class` of presence or absence. We'll then add our predictor variables[^9] to our model, with the formula `class ~ .`, equivalent to `class ~ bio4 + bio12`.
...
<!-- This could also be a footnote -->

Tuning is the process of simulating many different ways to fit lines made by our models to our training data. The number of curves in a model's line of best fit increases as a model becomes more complex, determined by its degrees of freedom. By using different functions to set reasonable weighting parameters that penalize a model when lines curve too much, tuning optimises the model's performance by finding the balance between fitting our current training data and predicting new values correctly. For more information, check out the [tunes package Getting Started vignette](https://tune.tidymodels.org/articles/tune.html).

<!-- I think I understand what you mean here, but my brain is trying to visualise it. Is there a way you can show that a models line of best fit increases as a model becomes more complex and when it curves substantially it is a negative outcome? I think what you mean is that the data that we have generated from our training data might be subject to some kind of error and to make sure it is in line with our real data we optimise it through tuning? OT  -->
...
#| warning: false
#| message: false
set.seed(234567) # for reproducability
...
<!-- What exactly am I looking for here? Logically my brain is saying the models with higher metrics are better? And.. a smaller range of the total line? Also they are ranked from left to right in order or model preference? Sorry if that is intuitive it is just a bit noisy. Looks like each graph are different metrics of assessment? OT -->
...
<!-- 
We can remove this section. The stacks website covers the same content pretty well
...
<!-- What was the conclusion here? That MaxEnt is the best model choice for our selected data? OT -->
...
<!-- How good our model is overall at making predictions of where kookaburras are likely to live this time? OT -->
...
<!-- Oh yes, how well it is at detecting a species presence or absence (class). Same thing lol OT -->
...
tidysdm offers [its own wrapper function]() [`simple_ensemble()`]() to run a stack model workflow and some [helpful ways](https://evolecolgroup.github.io/tidysdm/articles/a2_tidymodels_additions.html#exploring-models-with-dalex) to assess their performance.
:::
...
<!-- You might have to walk me through why suggesting more data isn't a good suggestion. I agree with the second part but haven't gotten to it because I feel it requires a few links/references. -->

<!-- I think the point I was trying to make about collecting more presence/absence data is based on thinking of this from a  research perspective, where it's expensive and/or just not possible to do any more sampling. It's almost the opposite cause/effect, where we're building a model because we can't get lots of data, versus let's get lots of data so we can build a better model. But saying it's not a good suggestion is too harsh; I'm happy for it to be one of the suggestions for improving the model. - SB -->

<!-- Just thinking again about how to imporve the model: do you think we need to be explicit e.g. with links/references? I was thinking we could leave it pretty broad like saying testing out different subsets of predictors and trying environmental layers at different levels of spatial resolution might improve the performance of the model, and if collecting more data is not an option then exploring other datasets that could be aggregated? - SB (again) -->
