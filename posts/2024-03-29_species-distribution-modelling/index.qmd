...
Species distribution models (SDMs) offer a way to quantify relationships between biodiversity observations and environmental variables, and are widely used in biogeography, conservation biology, and macroecology. SDMs allow us to assess how suitable geographic areas are for species to survive in, which has implications for predicting range shifts of invasive or threatened species, and understanding habitat suitability under different climate change scenarios.
...
<!--I personally found these simple examples much easier to understand than more complex examples based on real data when getting started (though I suppose I haven't decided how beginner-level these dropdown boxes should be. I've removed the second more convoluted plot and added a caption to reiterate that the colour is determined by the value assigned to the cell. If you have a more effective way to demonstrate what a raster is using bioclim variables then you are welcome to try, but I worry it will be too much effort - DK-->
...
Now we can use pair plots to view the relationship between each pair of variables. Variables `bio_12` and `bio_16` are very highly correlated (94%). To avoid multicollinearity in our model, we'll decide to only include `bio_12` (annual precipitation) and `bio_4` (temperature seasonality).
