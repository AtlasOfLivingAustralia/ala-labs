...
For many of the later steps we'll need to have the coordinates in a spatial data format to plot each point (i.e., `geometry`). So, let's convert our occurrence data to a spatial object (`sf`) defined by the longitude and latitudes coordinates, and set the CRS[^crs] to EPSG:4326[^epsg].
...
**I'm not sure that having two examples here is necessary to explain what a raster is, and I wonder if it might be better to explain this with a subset of one of the bioclim variables they've just downloaded** 
...
I disagree strongly with this paper's assertion that the effects of spatial thinning on model performance are unclear - just because the extent and type of thinning cannot be generalised across models and species does not mean the effects of thinning are unclear. If anything, this variation highlights the importance of testing different types of thinning and assessing model outputs instead of just choosing one and hoping for the best. There is an abundance of literature exploring this subject.

I also think that the main reason for thinning is to reduce spatial bias and autocorrelation, not make improvements to the speed of model fitting - SB
-->
...
Now that we have our presence and pseudo-absence points, the third step is to extract the environmental data for each point location in `aus` and bind the resulting values to our points data in `kookaburras_psuedoabs`. The result is a `tibble` with our points, their class, and the specific values of all 30 BioClim variables.
