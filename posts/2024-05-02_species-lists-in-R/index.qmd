...
```{r}
#| eval: false
threatened_list <- read_csv(here("example_TSL.csv"))
sensitive_list <- read_csv(here("example_SSL.csv"))
```
...
# big overall table
status_prop_table <- species_yass_grouped |>
  group_by(conservation_status) |>
  summarise(n = n()) |>
  mutate(proportion = n/sum(n)*100)

# taxa breakdown table
taxa_prop_table <- species_yass_grouped |>
...
# percent threatened
prop_threatened_or_sensitive <- status_prop_table |>
  filter(conservation_status %in% c("Sensitive", "Threatened")) |>
  summarise(total = sum(proportion)) |>
  pull(total) |>
  round(2)

# Waffle: Conservation Status
p1 <- ggplot(status_prop_table) +
  waffle::geom_waffle(
  ...
  scale_fill_manual(name = "Conservation\nStatus",
                    values = c("#F3E6DC", "#D89A98", "#AB423F"),
                    labels = c("No status", "Sensitive", "Threatened")) +
  labs(subtitle = glue::glue("{prop_threatened_or_sensitive}% species in Yass Valley are threatened or sensitive species"),
...
# Waffle: Taxonomic breakdown
p2 <- ggplot(taxa_prop_table) +
  waffle::geom_waffle(
    aes(fill = taxa_group,
        colour = taxa_group,
        values = n),
  ...
  scale_colour_manual(name = "Group",
                    values = c("#567C7C", "#22352C", "#465743", "#6D714A", "#C4AC79"),
...
For other posts, check out [our beginner's guide to map species observations](https://labs.ala.org.au/posts/2023-12-18_beginners-guide-make-a-map/index.html) or see [how to map distributions of invasive species](https://labs.ala.org.au/posts/2023-04-28_plotting-invasive-species/post.html).
