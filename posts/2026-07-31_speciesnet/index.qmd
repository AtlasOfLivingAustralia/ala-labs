...
We now have a few thousand images; however, not all ALA image records are equally suitable for SpeciesNet. The model was trained on camera-trap images and performs best on that type of photograph. Field photos, museum specimens, or heavily cropped images can produce unreliable predictions. Here, we can add a field to the `group_by` argument called `dataResourceName`, which will tell us the name of the datasets containing these images.  We can determine which ones are camera trap datasets from here.
...
Now we can see that only one dataset contains camera trap data: `Camera trap surveys in Queensland's Wet Tropics 2022-2023`.  As we are wanting a very small sample size for this example, we will restrict our query to see how many wild pig and feral cat images were captured by Camera trap surveys in Queensland's Wet Tropics 2022-2023 on December 17, 2022.
...
Now that we have a very small subset of images, we can retrieve image metadata as well as download the images using `galah.atlas_media()`. This function returns a table of records that includes the image URL (`imageURL`), species name (`scientificName`), data resource name (`dataResourceName`), and observation coordinates for every matching record (`decimalLatitude`, `decimalLongitude`). Crucially, setting `collect=True` means that we download the metadata *and* images.  To only get image metadata, set `collect=False`.  We will also specify that `path="EC_images"`
...
media_df = galah.atlas_media(
    ...
    collect=True,
    path="EC_images",
    progress_bar=False # set this to True if you want to see the progress of your downloads
)

print(media_df)
```

## Preview images before running {speciesnet}
...
# set up a path to find your images
...
The cell below loads SpeciesNet. The first time it runs, it will download the model parameters <!-- Dax, is this the correct word? --> (approximately 1–2 GB), which can take a few minutes. Subsequent runs in the same session use the cached parameteres instantly.
...
def print_predictions(predictions_dict: dict) -> None:
    ...
    column_names = ["Image ID","Class","Order","Family","Genus","Species","Species Epithet","Prediction Score"]
    ...
    for i,prediction in enumerate(predictions_dict["predictions"]):
        dict_for_table["Prediction Score"][i] = prediction["classifications"]["scores"][0]
        id_and_classification = prediction["classifications"]["classes"][0].split(";")
...
# Load a font 
font = ImageFont.load_default(size=16) # was 14  

# set plot parameters
n_show = min(10, len(image_paths))
fig, axes = plt.subplots(5, 2, figsize=(16, 24)) # 8, 12
...
All images in this run were either labelled *Sus scrofa* (feral pig) or *Felis catus* (domestic cat) in ALA, but SpeciesNet returned three different predictions: 
**domestic cat**, **wild biar**, and **coyote**. This is instructive.

These three species-level predictions — domestic cat, wild boar and coyote (*Canis latrans*) all belong to the same class, *Mammalia* (mammals). 

<!-- Dax, is this correct? -->
From a camera-trap image, a feral pig photographed side-on in low light can be visually similar to a large pig or a small cow, particularly when the image is partially 
obstructed or blurred. SpeciesNet's confusion between these species reflects a genuine visual ambiguity rather than a random error.

::: {.callout-tip collapse="true"}
# Classification of higher ranks
Any higher-rank predictions represent cases where the model could not confidently distinguish between species in that group and appropriately abstained from a species-level call. The fact that it still correctly identified the correct order and class demonstrates the model's graceful degradation under uncertainty.
:::

These predictions illustrates an important point: **SpeciesNet's output should be treated as a probabilistic first-pass label, not a definitive identification.** For research applications, predictions — especially those below ~0.9 confidence or at a higher taxonomic level — benefit from expert validation or a second-pass review.
