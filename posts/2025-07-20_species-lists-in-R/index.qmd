...
##### Species with more than one status {#sec-names-matching}
...
Whichever method you've followed, you will end up with very similar datasets containing threatened species and their statuses, though the number of matched species might differ[^different]. (due to taxonomic name mismatches explained earlier in @sec-names-matching).

[^different]: Due to differences in taxonomic names in the externally downloaded list and in ALA data. More info about this can be found under the "Names Matching" tab in the **Shapefile + list** section.
