...
# fire_extent <- rast(here("posts", "2025-04-02_greater-gliders", "fesmimgtif", "cvmsre_NSW_20192020_ag1l0.tif"))
```
...
-   1 - Reserved Experimental burn category (the use of this value varies state-wide and at present captures 'Non-[Fire Extent and Severity Mapping] burnt area' as a precautionary approach)
...
# Recode variable
burnt_cropped <- terra::ifel(fire_extent_cropped["fire_extent"] > 0, 1, 
                             terra::ifel(fire_extent_cropped["fire_extent"] == 0, -1, -1)
                             )
...
As final important check, we need to make sure there aren't rows that don't have a complete set of variable values. If one column has an `NA` value while every other column is filled, our models will inevitably fail (harsh, I know). A straightforward way to do this is to convert `combined_rasters` back to a tibble (for easy filtering) and check each column's `NA` values one-at-a-time like below. Eventually, we will get to the `burnt` column and notice that there are 2 rows that are complete except for the `burnt` column.
...
To fix this, when a variable column like `elevation` *isn't* `NA` and `burnt` *is* `NA`, then we'll make the `burnt` value `-1` (unburnt)[^notburnt], otherwise the value in `burnt` should remain the same. 

[^notburnt]: We are making the assumption that an unmeasured value is unburnt, which is ultimately not true but unknowable. It's best to fill those empty values with whatever the most conservative option is, or, even better, to impute those values based on the surrounding data.

```{r}
combined_rasters <- combined_rasters |>
  mutate(
    burnt = case_when(
      !is.na(elevation) & is.na(burnt) ~ -1,   # burnt
      .default = burnt
    )
  )
