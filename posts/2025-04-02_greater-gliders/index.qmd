...
<img src="https://images-assets.ala.org.au/store/2/1/0/8/e0bec186-b95a-4a50-8ce3-824143268012/thumbnail_large" class="rounded"/></img>

<img src="https://images-assets.ala.org.au/store/c/4/c/e/a0428bcf-f667-4f43-80b5-5ba1dc40ec4c/thumbnail_large" class="rounded"/></img>
:::
...
fire_extent_cropped <- rast(here("posts",
                         "data", 
                         "fire_extent",
                         "2026-03-18_fire-extent.tif"))

# fire_extent <- rast(here("posts", "data", "fire_extent", "fesmimgtif", "cvmsre_NSW_20192020_ag1l0.tif"))
```
...
```
class       : SpatRaster 
size        : 25094, 25094, 1  (nrow, ncol, nlyr)
resolution  : 9.962733e-05, 9.962733e-05  (x, y)
extent      : 148.5, 151, -37.50005, -35  (xmin, xmax, ymin, ymax)
coord. ref. : lon/lat WGS 84 (EPSG:4326) 
source      : spat_9234679a1911_37428_QYXbw8PZyYIJh7f.tif 
name        : Layer_1 
min value   :       0 
max value   :       5 
```
...
#| include: false
#| eval: false
# Documenting where lightweight raster used in this post is saved
terra::writeRaster(fire_extent_cropped,
                   here::here("posts", 
                              "data", 
                              "fire_extent", 
                              "2026-03-18_fire-extent.tif"))
...
#|   - "Fire Severity"
ggplot() +
  geom_spatraster(data = fire_extent_cropped, aes(fill = Layer_1)) +
...
-   1 - Reserved Experimental burn category (use of this value varies state-wide and at present captures 'Non-[Fire Extent and Severity Mapping] burnt area' as a precautionary approach)
...
For simplicity, let's recode `fire_extent_cropped` to whether an area is burnt or unburnt and save this info in a new column `burnt`. We'll use a dummy variable to make our results easier to interpret where burnt is `1` and unburnt is `-1`. To do this we'll use `terra::ifel()`, a version of `ifelse()` optimised for terra's SpatRaster objects. Then we'll rename the layer for simplicity.

Even after cropping, `fire_extent_cropped` is still massive; big enough that even simple operations like adding/editing a column can max out R's available memory and cause an error. To avoid this, we'll first run `gc()` to reduce R's used memory.
...
# Recode variable
burnt_cropped <- terra::ifel(fire_extent_cropped["Layer_1"] > 0, 1, 
                             terra::ifel(fire_extent_cropped["Layer_1"] == 0, -1, -1)
                             )
...
As a final but important check, we need to make sure there aren't any rows that *should* have a complete set of values for each of our raster layer variables, but are missing one or more values. If one column has an `NA` value while every other column is filled, our models will inevitably fail (harsh, I know). A straightforward way to do this check is to view each column's `NA` values one-at-a-time to visually search for obvious errors. Doing so will eventually show that 2 rows in `combined_rasters` are complete except for the `burnt` column value[^the-other-columns].

[^the-other-columns]: Other rows shown below are majority or all `NA` values except for their coordinates. These should be ignored by our model. However, it's worth being aware that these rows exist in our data in case the model does fail. In that case, they might need further data filling or removing.
...
To fix this, we'll use `case_when()` to conditionally add data to `burnt`, such that when the value in `elevation` isn't `NA` *and* the value in `burnt` is `NA`, then we'll make the `burnt` value `-1` (unburnt)[^notburnt]. Otherwise the `burnt` value should remain the same. 

[^notburnt]: We are making the assumption that an unmeasured value in `burnt` is an area unburnt by fire, which is ultimately not true but also unknowable. It's best to either fill empty values with whatever the most conservative option is, or, even better, to impute those values based on the surrounding data. In this case, given we're not interested in the specific locations, our patch-up job should be enough.

```{r}
combined_rasters <- combined_rasters |>
  mutate(
    burnt = case_when(
      !is.na(elevation) & is.na(burnt) ~ -1, # unburnt
      .default = burnt
    )
  )
```
...
Running our check again shows that those rows are no longer returned when we filter for `NA` values in the `burnt` column.
...
A quick first impression tells us that `fire_period` and `burnt` variables are significant but quite weak predictors. You might also notice that some variables like `elevation` are very significant in the model but have tiny estimate coefficients. Let's use DALEX to help us understand how this is possible.
...
-   The negative estimate of `fire_period` is very weak, but it suggests that there are more records of greater gliders after the fires than before[^afterfires]. This is probably due to the growing number of records each year in the ALA.
-   The slight positive estimate of `burnt` is also very weak, but it suggests fewer records in burnt areas[^burntareas].
-   The estimated effects of `fire_period` and `burnt` on our model aren't sufficient enough evidence for us to confirm our hypotheses that areas burnt by fire have fewer possum occurrences, given the data. 
