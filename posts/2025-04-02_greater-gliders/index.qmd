...
As a final but important check, we need to make sure there aren't any rows that should have a complete set values for each of our raster layer variables but are missing one or more. If one column has an `NA` value while every other column is filled, our models will inevitably fail (harsh, I know). A straightforward way to do this is to check each column's `NA` values one-at-a-time to visually search for obvious errors. Doing so eventually shows that 2 rows in `combined_rasters` are complete except for the `burnt` column value.
