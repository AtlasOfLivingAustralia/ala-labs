...
To see the more traditional statistical test results (e.g., estimates, test statistics, p-values), we'll have to extract them from our Generalised Linear Model (GLM) which we ran over our tuning process. 
...
Our tuning process earlier identified which workflows (model + parameters) worked best for our data, saved in `gliders_models_tune`. We can use `rank_results()` to see which ones performed best based on the Boyce Continuous Index.
...
We can also see the partial effects of our model, which show how the model is fitting each individual effect. Here we can appreciate the model's complexity, as each variable has its own non-linear fitted line!
