...
description: |
  Box plots are a common way to summarise data in ecology and biology research, but box plots have their weaknesses. Here we'll show how easy it can be to make beeswarm and raincloud plots—two alternative plots with greater data transparency—using plant trait data from {austraits}.
author:
  - name: "Dax Kellie"
...
Now we can add our boxplot, using our `after_stat()` colorspace trick within `geom_boxplot()`, this time to darken the lines of each box. We'll also add use {pilot} to add some nice colours and styling to our plot.
