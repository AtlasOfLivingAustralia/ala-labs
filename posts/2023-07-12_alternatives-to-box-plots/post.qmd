...
Leaf mass per area is a morphological trait that can indicate a plant's survival strategy, and values of LMA can vary widely across species:  
...
Let's briefly revisit leaf mass area before ending. The relationship between LMA and plant functioning means we can *infer* the type of ecosystem a plant lives in based on its LMA. So let's see where the 7 plant species that we summarised above live! 
