...
Buffers are used because many types of species are mobile or seasonal, and their presence/absence fluctuates depending on the time and location. So to get a more accurate estimate of what might live in a specific location (and you already know this), people can't just get the species observations returned in a shapefile. They also need to consider what has potential to live in an area by checking what might live just outside of the area (and therefore might be in our area at a different time in the past or future), and any pressures external to the specific area that might influence their area (imagine a wetland next to an industrial area - what kind of weird weeds might grow there...). 

So adding a buffer doesn't *only* refer to adding a circular buffer around a given area (though that is common). Sometimes it can mean considering biodiversity corridors, where connections to forests or rivers might offer spaces where species or populations can arrive (or exit) a given location (this is called a movement corridor apparently). Importantly, depending on the types of species you are interested in capturing, the relevant buffer size might differ. For example, birds are incredibly mobile so a larger buffer might be required to capture the full diversity of birds. Other things like pollen, spores or invertebrate larvae (ie dragonfly nymphs) can be quite mobile, and can travel to new locations via wind or rivers.

In other words, we are trying to get across that because each ecosystem is different, people should give some thought about what buffer size is optimal for a given investigation. The effects of this might matter if they are used for modelling, (which is mentioned in the intro but perhaps without context that's a little confusing) or even more so if they are used for area assessments prior to land development. The concepts are not difficult, but their implications might be. Ideally this post could even help start the thinking process with some examples.
