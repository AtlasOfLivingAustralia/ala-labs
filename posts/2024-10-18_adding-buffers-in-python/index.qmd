...
In this post, we'll show how to add a buffer around a shapefile with `{geopandas}`, `{shapely}` and `{matplotlib}`<!-- links to these package websites?-->. Then we will use `{galah-python}` to download data of Stuttering frogs (*scientific name here*<!-- please add -->) to demonstrate how the size of a buffer can affect the detection of sensitive species in an area.

# Draw a buffer

For our example, our area of interest is Mid-Western, a Local Government Area (LGA) in Western Australia. We'll first need to get a shapefile of our area, which we can get by downloading a [shapefile of all LGAs from the Australian Bureau of Statistics](https://www.abs.gov.au/statistics/standards/australian-statistical-geography-standard-asgs-edition-3/jul2021-jun2026/access-and-downloads/digital-boundary-files). Download the zip file of "Local Government Areas - 2024", then place the zip file in your local directory and unzip it.

<!-- can you provide a small map of where this is like we have in other labs posts? See here:  https://labs.ala.org.au/posts/2023-12-18_beginners-guide-make-a-map/#download-observations-->
...
<!-- 
Dax: 
I'm not sure people are going to find this section particularly useful to the target audience. You haven't showed people how to make one buffer, but rather how to make many of them! Probably overkill without showing the simple task first. Can you make the 'many-buffers' plot a final example and hide the code in a folded chunk using `#| code-fold: true`? I am also debating whether it's necessary at all given the sensitive species example is meant to show a similar but more complex concept. 

It might also be useful to show how to use a buffer to do one or both of these things:
 (A) Filter point observations to only those in your buffered area
 (B) Use a buffered shapefile with `galah_geolocate()` + `atlas_species()` (though this option is far less accurate, and probably doesn't capture plants and inverts accurately, so maybe it's the least important)
-->

## Comparing buffer size

Now, we will create different sized buffers.  This will involve converting the shapefiles between different Coordinate Reference Systems (CRS).

<!-- 
Thanks to our beginners post on maps, we can just link to our explanation of what CRS is rather than constantly re-explain it. Here's an example of how I usually link to it:

Check our [this section of a previous ALA Labs post](https://labs.ala.org.au/posts/2023-12-18_beginners-guide-make-a-map/#make-a-map) for a more complete explanation of what a CRS is.

----

I also suggest adding the CRS information below in a footnote, like we've done previously here:
https://labs.ala.org.au/posts/2024-04-30_sdm-tidymodels/#:~:text=set%20the%20Coordinate%20Reference%20System%20(CRS)%20to%20EPSG%3A43261%20(WGS84)
...
Next, let's see an example of how buffer size affects our ability to detect sensitive species accurately.

#### Obfuscation

<!-- 
I don't think at any point we really demonstrate what "obfuscation" means, both at a data level, and at a spatial level (ie on a map). Would you be able to explain here that obfuscation is
-->
...
Sensitive species in the ALA are obfuscated by an amount specified by the state it is found. Thankfully, the ALA has up-to-date sensitive species list from each state and territory.  You can find out these values in `{galah-python}`.  First, however, we have to find the ID of the New South Wales sensitive species list.
...
<!-- Phew this is a lot of code and code comments. I always find that for explanatory posts, this is a little overwhelming for beginners. Are you able to break this up into sections and explain the general steps? I had to do this a lot in Jarod's post if you are looking for an example-->
...
<!-- This is a serious amount of code, and it includes a loop. I imagine people who read this post might want to know how to do a single basic plot first and foremost, then more expert coders might want to try this hectic dataviz. Maybe it's best to fold this code chunk and just explain what it shows (something that I think is missing here). What do you think is the best buffer size given this example here? Does it depend on what species we care about? Would an Impact Assessment for an industry project have a different buffer size to a conservation project? (you don't necessarily have to answer all of these, but I imagine these are the questions someone might have. We can talk about this if you aren't exactly sure of the answer) -->
