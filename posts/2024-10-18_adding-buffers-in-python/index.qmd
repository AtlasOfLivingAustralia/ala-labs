...
<!--
Dax: Sorry Amanda, but I'm once again going to suggest a slight change on this to make the example a little more realistic. The example here is true, but where things aren't quite right is in your example, the true data point is always in the centre of the radius. This isn't actually what happens, and this example we have a little more control because we are only illustrating one point in each map.

So, could you instead put the yellow radius in a certain section of your map and *don't move it*. Instead, move the two dots inside the radius to suit each of our three scenarios. This will help people get their heads around how the points in the radius could literally end up anywhere!
-->
...
Let's try visualising the uncertainty around these points. The `generalisationInMetres` column shows that each frog record is obscured by 10 km (10,000 m). To make this more clear, we'll draw a radius around each point. This isn't a perfect representation of obfuscation---the true location could be anywhere inside 10 km, not directly in the middle of each circle---but it gives an idea. 

<!--
Dax: From this point to the Final Thoughts section, treat it like another example (like you did above), where the code is in a folded code chunk. 
On a reread, I've realised the end goal of this article isn't to show people how to make 6 maps with these radius-points around them, the goal is to show people to then download data with galah and filtering to inside the buffer size they decided on.

So, as it is now, this is yet another wonderful example, but it feels like we didn't finish what we promised at the start (show people how to use a buffer to understand what species are in a location) because we never downloaded all the data in the area and summarised it.

Could you please rephrase this as an example, meaning any description of code can be deleted. Then could you please do this in a final section (I've added a header)
-->

<!-- you can remove the text below -->
To do this, we'll remove any duplicate records[^duplicates] and records missing a location, then convert our `DataFrame` to a spatial `GeoDataFrame` projected to CRS `4326`[^crs_again].

[^duplicates]: Confusingly, these points might not be *actual* duplicates---they just appear that way because their obfuscated locations now overlap on the same point. Their true locations, however, may be in different places.

[^crs_again]: Again, this is the CRS ALA data is projected to. 
...
# Download data in our buffered area

<!-- 
Dax:
Here you can download records in your buffer however way is most efficient. Then you can summarise and visualise them on a map (density point map + summary table of species? Maybe in 2 tabs?). Doing this will help the article feel complete :)
-->
