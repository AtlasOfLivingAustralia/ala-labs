...
threat_species.head(10)
```

We then narrow the number of observations to only the buffered area around Midwestern. 
...
# get all species within a 10km buffer
...
# code to make map
fig, ax = plt.subplots()
ax.scatter(threat_species_10km['decimalLongitude'],threat_species_10km['decimalLatitude'], alpha=0.3, color='#d4af37')
...
# code to make map
fig, ax = plt.subplots()
a = ax.scatter(threat_species_10km['decimalLongitude'],threat_species_10km['decimalLatitude'], alpha=0.3, color='#d4af37')
...
ax.barh(n + h/2, num_species_mid_sorted.values(), height=h, color='#292C26', label='Midwestern')

# set ticks
ax.set_yticks(range(len(num_species_mid_sorted.keys())), labels=num_species_mid_sorted.keys())
ax.barh(n - h/2, num_species_buff_sorted.values(), height=h, color='#358BA5', label='Buffer')
ax.legend()
ax.set_xlabel('Number of Unique Species')
ax.set_aspect(aspect=12)
```

Finally, we can plot the bar plot and map side-by-side.  This will show: i) that the number of threatened species detected markedly increases when adding a buffer, and ii) adding a buffer includes other hotspots of threatened species that would otherwise have been missed had we just considered the Midwestern LGA.

```{python}
#| code-fold: true
#| echo: true
#| eval: false
#| code-summary: Code to make side-by-side plot
# import numpy for arange
import numpy as np
...
ax[0].barh(n - h/2, num_species_buff_sorted.values(), height=h, color='#358BA5', label='Buffer')
ax[0].legend()
