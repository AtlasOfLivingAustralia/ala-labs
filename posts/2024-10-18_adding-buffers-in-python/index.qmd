---
title: "Add a buffer to a shapefile"
description: |
...
# Threatened Species with/without a buffer

We've considered our buffer size, so let's go with a 10km buffer and download occurrence records in our area. To understand how much our buffer impacts the number of sensitive or threatened species we return in Midwestern, let's download data for this region and compare how many threatened species we detect with and without a 10km buffer.  First, let's draw a 10km buffer around the Midwestern LGA, and then draw a bounding box around this buffer.  We will then query by this bounding box, as it is more efficient to query in `galah-python` using a bounding box.
...
There are a lot of observations in the area!  We are only interested in species mentioned in the EPBC Act, so we'll merge our records to the list of species names, which filters out species that don't have a conservation status.  We'll also use `show_values()` to download a full species list and include any original columns in the species list using `all_fields=True`, which in this case adds status information. We will then determine which species are in Midwestern and which are in the buffered region.

```{python}
# get all species on epbc list + status info
epbc_list = galah.show_values(field='dr656',lists=True,all_fields=True)

# merge epbc list with species list to get only threatened species
threat_species = pd.merge(species,epbc_list[['scientificName','status']],on='scientificName')

# convert to GeoDataFrame for straightforward spatial filtering
...
<!-- Dax proposed structure -->

We've considered our buffer size, so let's go with a 10km buffer and download occurrence records in our area. As a final step, let's visualise where threatened species have been observed in our buffered area on a map. To start, let's download occurrence records in our area with `galah-python`.

```{python}
# chunk making buffer and downloading records with `atlas_occurrences()`
```

Next we can use `search_all()` to find the EPBC Act Threatened species list. Then, we'll use `show_values()` to download the content of that list. By adding `all_fields=True`, we can all original columns of the list, which in this case includes status information.
...
By merging our species list to our occurrence records `df-name`, our data will filter to only species on the EPBC list.

```{python}
# merge + result
```

Finally, we can plot our observations on a map, adjusting the opacity of our points (`alpha`) so we can see areas with many overlapping points easier. To understand just how many additional species our 10km buffer captures, we've added an additional bar plot showing comparing species in Midwestern vs in our buffer.
...
# place both final map objects here. You can do your subplot method or quarto can do it for you using the code chunk options I've added
