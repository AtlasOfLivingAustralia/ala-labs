...
<!-- I think you can delete this chunk
...
# Download data for EPBC Act threatened species in our buffered area

<!-- 
This dataviz is honestly great, but I think it's overkill for the end of this post. I think there is enough content here for another Labs post on this.

To finish, I think we need to show people how to do the things they would need for their summary. Bar plot + simple map should do it, as discussed
-->


Now that we have considered how adding buffers to an area might affect sensitive or threatened species, we can download data for this region and compare how many threatened species we detect with and without a buffer.  First, we are going to draw a 15km buffer around the Midwestern LGA, and then draw a bounding box around this buffer.  We will then query by this bounding box, as it is more efficient to query in `galah-python` using a bounding box.

<!-- I think you should at least add the ALA data profile onto your query to make sure the points aren't obviously erroneous -->

```{python}
#| echo: true
#| eval: false
# make a 15km buffer
buffer = midwestern_metres['geometry'].buffer(15000)
gdf_buffers_degrees = buffer.to_crs(4326)
buffer_15km = shapely.unary_union(gdf_buffers_degrees)

# get bounding box around buffer for efficient querying
bds = buffer_15km.bounds
...
After narrowing our occurrences by the EPBC Act, we will now look at our two sets of data (using only the Midwestern shapefile or using the 15km buffered shapefile), and determine how many unique species of each status were found in each shape.
...
threat_species_15km = threat_species[threat_species_gdf.geometry.within(buffer_15km)]

# set dictionary for getting number of species sorted by status
num_species_mid = {x:0 for x in list(set(species_mw['status']))}
```

Now, we can determine the number of unique species for each status within either the Midwestern shape or the 15km buffered shapefile.
...
# set dictionary for getting number of species sorted by status
num_species_buff = {x:0 for x in list(set(threat_species_15km['status']))}

# loop over each key and get the number of unique species for each status in buffered region
for key in num_species_buff:
    temp = threat_species_15km[threat_species_15km['status'] == key]
...
To create our final plot, we will first calculate the density of occurrence points in space.  This will help determine if there are any 'hotspots', i.e. if there are overlapping occurrence points within the same point in space.  What this will also show is if there is any potential to miss threatened species.

```{python}
# import scipy for point density
import numpy as np
from scipy.stats import gaussian_kde

### KDE Chart ###
# put your lat/longs into numpy arrays for point ensity
x = np.array(threat_species_15km['decimalLongitude'])
y = np.array(threat_species_15km['decimalLatitude'])

# Calculate the point density
xy = np.vstack([x,y])
z = gaussian_kde(xy)(xy)

# sort from lowest to highest for plotting
idx = z.argsort()
x, y, z = x[idx], y[idx], z[idx]
```

After all of this analysis, we are now ready to create our final plot!

```{python}
#| echo: true
#| eval: false
# create color map
cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["white","#62A769", "#52AA5E", "#5A2A57"])
...
ax[0].set_aspect(aspect=12, anchor=(0,0.89))

# density map
cax = ax[1].scatter(x, y, c=z, s=50, cmap=cmap)
ax[1].plot(*midwestern['geometry'][74].exterior.xy,c='#292C26',lw=2)
ax[1].plot(*buffer_15km.exterior.xy,c='#358BA5',lw=2)
plt.colorbar(cax, orientation='horizontal', ticks=[], label='Number of Overlapping Points', anchor=(0,2.3))
plt.axis('off')
plt.suptitle('Threatened Species in Mid-Western and Surrounds', fontsize=24, y=0.93)
ax[1].set_aspect(aspect='equal', anchor='C')
```
...
a = ax[0].set_aspect(aspect=12, anchor=(0,0.89))

cax = ax[1].scatter(x, y, c=z, s=50, cmap=cmap)
a = ax[1].plot(*midwestern['geometry'][74].exterior.xy, c='#292C26', lw=2)
a = ax[1].plot(*buffer_15km.exterior.xy, c='#358BA5', lw=2)
a = plt.colorbar(cax, orientation='horizontal', ticks=[], label='Number of Overlapping Points', anchor=(0,2.3))
a = plt.axis('off')
a = plt.suptitle('Threatened Species in Mid-Western and Surrounds', fontsize=24, y=0.93)
a = ax[1].set_aspect(aspect='equal', anchor='C')
a = plt.savefig('final_plot.png')
plt.show();
```

What we can see here is that without the buffer we added, we would have missed quite a large number of threatened species, including a hotspot in the lower right-hand corner of the map.
