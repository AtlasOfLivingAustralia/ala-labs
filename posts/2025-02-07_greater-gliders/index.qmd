...
description: |
  Major fire events impact flora and fauna, particularly those that dwell in areas where fire dramatically alters the livable habitat area. Here we investigate how greater gliders, a tree-dwelling marsupial species, were impacted by the 2019-2020 bush fires using {tidymodels} and {tidysdm}.
author:
  - name: "Jarod Wright"
  - name: "Dax Kellie"
date: "2025-02-10"
...
In 2019-2020, Australia experienced one of the most catastrophic bushfire seasons on record, burning approximately [19 million hectares](https://wwf.org.au/what-we-do/australian-bushfires/), of which 12.6 million were primarily forest and bushland. An estimated [900 species](https://www.nature.com/articles/s41586-024-08174-6) of plants and animals were severely impacted, and [3 billion animals were killed or displaced](https://wwf.org.au/what-we-do/australian-bushfires/).
...
If there are a few tailing decimal places that appeared while converting the `extent` of `fire_extent_cropped`, we can crop it again using `bbox_ext` to make sure it matches other rasters. You'll see the `extent` slightly but noticeably changes.
...
Now would be a good time to save your new cropped raster layer locally. This resulting cropped raster layer is *much* smaller than our original layer, and it will be much easier (and faster) to use this smaller layer in your workflow. Just be sure to document where you downloaded the larger file and the process you used to crop it!
:::
...
For simplicity, let's recode `fire_extent` to whether an area is burnt or unburnt and save this info in a new column `burnt`. We'll use a dummy variable to make our results easier to interpret where burnt is `1` and unburnt is `-1`. Then we'll rename this layer for simplicity.
...
#| warning: false
#| message: false
combined_rasters <- c(elevation_aligned, temp, precip, burnt, tree_cover)
...
[Earlier](#before-or-after-fire), we categorised our greater glider observations as `pre_fire` or `post_fire`. We will need to categorise our pseudo-absences into the same categories to allow our model to generate a probability of presence in both conditions, pre- and post-fire.
...
To see the more traditional statistical test results (e.g., estimates, test statistics, p-values), we'll have to extract them from one of our Generalised Linear Models (GLM) run over our tuning process. 

:::{.callout-note collapse="true"}
##### Why do we have to use GLM results?
Generalised Linear Models fit a straight line of best-fit to the data. As a result, it's possible to return a coefficient that estimates the slope of the line and some test statistics about its importance in the model.

Alternatively, other models we've run like Maxent and Random Forest models are a little different. They use weights to "penalize" a model to get a better fitted line; one that isn't just straight, but shaped to be a better fit to the data. This shaped, squiggly line can be represented by a unique equation, not just a coefficient of slope. When asked to return results for models like these, tidymodels returns penalty values (*lambda*) used by the model, not a summary of statistics and *p* values. 

This means we have to use other packages (like vip and DALEX) to assess more complex types of models like our complete `gliders_stacked` model.
:::

### Retrieve GLM results

Our tuning process earlier identified which workflows (model + parameters) worked best for our data, saved in `gliders_models_tune`. We can rank which ones performed the best based on the Boyce index.

```{r}
# Return the models that performed best
gliders_models_tune |>
  rank_results(
...
To use the GLM candidate model, we need to extract that model's workflow (which includes the recipe and any case weights or hyperparameters assigned through the tuning process).
...
Then we can use this `best_model` to fit our complete `gliders_filtered` data.
...
Before we see the results, let's remind ourselves what we are expecting to see given our hypothesis:

-   Fewer occurrences after the fire. This should appear as a significant, negative `fire_period` coefficient.
-   Fewer occurrences in burnt areas. This should appear as a significant, negative `burnt` coefficient.
-   An interaction between fire period and burnt area, because burnt areas after fire will have very few records. This should appear as a significant, negative coefficient.

Let's view our model's results! We can return them using `broom::tidy()`.
...
Let's see which parts of our model had the greatest importance for predicting the outcome. You'll notice that the results looks similar to the results of our GLM.
...
DALEX can return many other helpful summaries to check model performance and results. [See more here](https://bookdown.org/gaetan_lovey/data_analytics/dalex.html).
:::
...
-   The positive estimate of `burnt` suggests fewer records in burnt areas[^burntareas]. However, the effect is pretty small.
