...
To start, we will use the infamous example of the cane toad to illustrate how far an invasive species' distribution can spread.
...
Now, we need to clean our data to ensure that there are no null or missing values in the spatial and date fields we will use in our visualisation (`eventDate`, `decimalLatitude`, and `decimalLongitude`). Because `galah.atlas_occurrences()` returns a Pandas dataframe, we have plenty of functions we can use to clean our data.

```{python}
cane_toads = cane_toads.dropna(subset=["eventDate", "decimalLatitude", "decimalLongitude"])
```

We will be mapping cane toad distribution for each decade; however, the `eventDate` value for each record is a string value in the format `yyyy-mm-dd Thh:mm:ssZ`. We have written our own function titled `convert_date_to_decade()` to take a date represented as a `string` as input and return its corresponding decade. It does this by extracting the year from the date string and then rounding it down to the nearest decade.
...
Next, we need to download a shapefile of Australia with state boundaries to be able to plot species distributions onto. The Australian Bureau of Statistics provides [digital boundary files](https://www.abs.gov.au/statistics/standards/australian-statistical-geography-standard-asgs-edition-3/jul2021-jun2026/access-and-downloads/digital-boundary-files) from which you can explore many other Australian shapefiles. Download the [States and Territories - 2021 - Shapefile](https://www.abs.gov.au/statistics/standards/australian-statistical-geography-standard-asgs-edition-3/jul2021-jun2026/access-and-downloads/digital-boundary-files/STE_2021_AUST_SHP_GDA2020.zip) a zip folder. Save it inside your working folder and then unzip it to access the `.shp` file inside.
...
One good example of native pests are *Pittosporum undulatum* trees. These trees have been introduced as ornamental plants in gardens across Australia because of their sweet-scented flowers and bright berries. Although *P. undulatum*'s native range extends from southern Queensland to eastern Victoria, it is now considered an environmental weed in many regions where it has been introduced.
...
```{python}
#| eval: false
bioregions = geopandas.read_file("path-to-folder/IBRA7_regions/ibra7_regions.shp")
bioregions.plot(edgecolor = "#5A5A5A", linewidth = 0.25, facecolor = "white")
```
...
Human activity—from constructing buildings to travelling overseas to gardening—plays part in shaping modern ecosystems. Our maps showed how quickly well-known invasive species have established themselves across Australia, and how widely even native Australian plants can spread when introduced to non-native regions.

Humans are just one of many drivers of introducing species to new areas. Changes to the environment, for example, can shrink available resources and living space in a habitat. Introduced invasive species that are more flexible to environmental changes (and many well-known ones are) often outcompete native species for these available resources. [Large weather events, extreme temperatures and habitat degradation](https://theconversation.com/research-reveals-fire-is-pushing-88-of-australias-threatened-land-mammals-closer-to-extinction-185965) can give invasives a big leg-up on the native competition as [species inevitably enter and alter ecosystems](https://soe.dcceew.gov.au/overview/pressures/invasive-species-and-range-shifts#range-shifts-and-extensions). 
