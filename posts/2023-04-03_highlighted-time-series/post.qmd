...
To avoid this, let's *scale* our record counts by the total number of *Anatidae* observations each year. Doing this let's us compare proportions rather than raw numbers. 
...
To compare observations in years prior to COVID-19 to observations in 2020 and 2021, we'll want to plot 2 lines: 

  1. A baseline of average observation counts in 2017-2019 
...
We have two separate columns in `birds_mean_prop` containing proportional counts (`year_2020` and `year_2021`). Although there are 52 weeks in a year, our counts extend from weeks 1-53 because 2020 didn't start or end *exactly* at the end of the week — 2020 ended on a Thursday and 2021 ended on a Friday. 
...
p2 <- p1 +
  ...
  labs(y = "Proportion of year's total observations",
       ...
       subtitle = "Proportion of total records each year (2017-19 vs. 2020-21)")
p2
```
...
p2 + 
  ...
  annotate_pilot(label = "2021", x = 78, y = 0, 
                 alpha = 0.7, vjust = 3.8, size = 10) +
  labs(subtitle = "Proportion of total *Anatidae* records each year (2017-19 vs. 2020-21)") +
  theme_pilot(grid = "",
              axes = "bl") +
