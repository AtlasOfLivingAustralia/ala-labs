...
description: |
  Knowing what species have been observed in a local area is a regular task for ecosystem management. Here we show how to make a species list and how to cross-reference this list with threatened and sensitive species lists. We show how to do this task using 2 methods, with {galah-python} and using an external shapefile and species list. We then show how to visualise this information as a bar chart and a waffle chart using {plotnine}, {matplotlib} and {pywaffle}.
author:
  - name: "Dax Kellie"
  - name: "Amanda Buyan"
date: "2025-07-13"
...
py_install("../../../galah-python/galah",pip=TRUE,ignore_installed = TRUE)
py_require(c("galah-python@/Users/buy003/Documents/GitHub/galah-python/galah"))
py_require(c("numpy"))
...
# read in shapefile
lgas = gpd.read_file('../data/LGA_2025_AUST_GDA2020.zip')
...
uninc_nsw = lgas_wgs84[lgas_wgs84['LGA_NAME25'] == 'Unincorporated NSW']
exclude_box = shapely.box(110,-45,155,0)
lgas_wgs84[lgas_wgs84['LGA_NAME25'] == 'Unincorporated NSW'] = uninc_nsw.clip(exclude_box)
lgas_wgs84 = lgas_wgs84[lgas_wgs84['geometry'].within(exclude_box)]

# get shoalhaven
shoalhaven = lgas_wgs84[lgas_wgs84['LGA_NAME25'] == 'Shoalhaven']
...
Next we'll filter our shapefile to Shoalhaven. The column `LGA_NAME25` contains area names, and we can filter our data frame to only rows where `LGA_NAME25` is equal to `Shoalhaven`. We are left with a single polygon shape of Shoalhaven.
...
For Python, we will be downloading [Roboto](https://fonts.google.com/specimen/Roboto) from Google Fonts, saving the folder in your current directory, unzipping the folder, and loading it with the matplotlib library.
