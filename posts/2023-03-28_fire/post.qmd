---
title: "Quantifying species range overlap with fire-burned areas using concave hulls"
description: |
  Calculating range overlaps with concave hulls is an efficient way to estimate the impact natural disasters have on biodiversity. Here we'll use curated datasets to compute concave hulls and use these to visualise the range overlap of *Apidae* (Bees) and *Daviesia* (Bitterpeas) with the burned areas of the Black Summer fires of 2019-2020. 
author:
  - name: "Fonti Kar"
...
[Margot Schneider](https://labs.ala.org.au/people/Schneider_Margot/index.html)
...
:::{.announce style="background-color: #FFEAC2; text-align: center;"}
...
Here, we show you how to compute a new form of spatial polygon --- a [*concave hull*](https://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.676.6258&rep=rep1&type=pdf) --- and use it to estimate and visualise species-range overlap with burned areas. Unlike convex hulls, concave hulls have the added flexibility to adjust their tightness to the data (or *concavity*). This flexibility allows more accurate estimation of species ranges, making it a useful approach to rapidly assess how natural disasters like bush fires impacted biodiversity.
...
A total of 44,146 invertebrate species and 896 species of vascular plants are represented in these datasets. Below, we've attempted to show the geographic range of these data. So much data makes it challenging to visualise all data points and concave hulls at once, so to create the concave hull maps (left) below we randomly selected one invertebrate species from each *class* and one plant species from each *family*.
...
The invertebrate dataset includes records on offshore islands which can drastically skew the shape of a species' concave hull. For the purpose of this post, we will filter these out and only use data for records that occur on mainland Australia and Tasmania.
...
Shapefiles for the 2019-2020 fire season are available through the [National Indicative Aggregated Fire Extent Dataset](https://www.environment.gov.au/fed/catalog/search/resource/details.page?uuid=%7B9ACDCB09-0364-4FE8-9459-2A56C792C743%7D) from the Department of Climate Change, Energy, the Environment and Water. Click on **Download Data** (near the top of the page), then click on **NIAFED_v20200623.zip** to download the zip file. Save the zip file in your project folder and unzip to retrieve the shapefiles.
...
Using `st_intersection()`, we can identify the overlap between each species' concave hull with fire-burned areas. We can then use `st_area()` to calculate the area (m^2^) of overlap and convert it into a percentage of each species' original range so all species are comparable.
...
<!-- Dax, can you please help us crop this first bee please so the circles are alike? Thanks!!! -->
...
This map shows the three bee species with the highest percentage overlap with burned areas. Our map shows that two of these bee species are located in Northern Australia and one is located in South-Eastern Australia.
...
In natural catastrophes, decision makers have limited time to act. To choose the best reponse they need ready-to-go data and workflows to assess possible ramifications of their actions and inactions. Here, we used [curated datasets](https://data.csiro.au/collection/csiro:56679) of Australian invertebrates and vascular plants to illustrate how concave hulls can represent estimate species range overlap with natural disasters. We hope our work can aid future assessments of vulnerable species and help prioritise conservation efforts.
