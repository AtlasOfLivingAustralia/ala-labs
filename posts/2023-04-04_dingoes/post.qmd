...
:::{.callout-tip}
If we search {galah} for CAPAD (`search_all(fields, "capad")`), we'll find that {galah} has the CAPAD 2020 layer available for us to use to filter our data (field ID `cl11033`). We can return counts of dingo observations in CAPAD areas only using `galah_group_by()` in our query. Then by adding the record counts together we'll return the total number of observations in CAPAD areas.
:::
