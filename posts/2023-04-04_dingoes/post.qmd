...
:::{.callout-tip}
If we search in {galah}, we'll find that {galah} has the CAPAD 2020 layer available for us to use to filter our data (field ID `cl11022`). We can return counts of dingo observations only in CAPAD areas by adding `galah_group_by()` to our query, then add them together to find the total records in CAPAD areas.
:::
...
Overall, in the Northern Territory and South Australia, the number of dingo observations is fairly comparable between between those made inside and outside of nationally protected areas. We also saw how few observations are on the southern side of the Dingo Fence in South Australia.
...
[^1]: However, all wild dogs in Australia [possess some percentage of dingo genes](https://newsroom.unsw.edu.au/news/science-tech/dogs-not-gone-wild-dna-tests-show-most-wild-dogs-australia-are-pure-dingoes).
