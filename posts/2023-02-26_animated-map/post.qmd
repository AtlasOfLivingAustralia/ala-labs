...
In order to do this, we'll create a set of custom functions that:
  ...
  1. Filter observations to only those of a specific month, 
...
To build our overall SDM we've chosen to use [the method used by Liam Bailey in his Bigfooot map](https://github.com/LiamDBailey/TidyTuesday/blob/93cbe1143757c97ddaf36c564ac03a2b8088cb56/R/2022/Week37_2022.qmd). It's a fairly flexible model that suits our purposes to quickly see where nudibranchs are observed around Australia.

We'll build another custom function to run these models called `run_sdm_model()` for each chosen month.

:::{.callout-note}
This SDM method merges the results from several models into one final value using [Fisher's combined probability](https://en.wikipedia.org/wiki/Fisher%27s_method). It's is by no means the most robust SDM. If you are trying to make a more informative species distribution model, it might be worth considering [other methods](https://www.frontiersin.org/articles/10.3389/fmars.2017.00421/full)!
:::
...
Let's also make a nice colour palette for our map:
...
Let's zoom in on just the east coast so we can see the details. We'll once again make a custom function `make_the_map()` to construct each map. Other than making a nice plot, this adds each month's 3-letter abbreviation `month_label` to the top of each map for our eventual animation.
...
Now we can save our maps. We'll assign our plots a letter so they are ordered alphabetically and saved in order in a new folder called `maps`.
...
It looks like there are nudibranchs along pretty much the entire coastline of Australia!
...
Distributions predicted using fewer observations are more strongly influenced by outliers. In our case, some months have more observations than others - there are more observations of nudibranchs from October-January, and fewer from May-August - but it's not easy to tell which months those are in our animation! It's very possible this had an effect on the patterns we see in our final GIF.
