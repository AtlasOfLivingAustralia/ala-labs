...
description: |
  Time series analyses can be handy for seeing trends over time, and exploring how trends might relate to major events. Here, we show how to create an exploratory time series plot comparing observations of waterbirds prior to and during COVID-19.
author:
  - name: "Thai Rushbrook"
...
A majority of species observations in the Atlas of Living Australia are collected opportunistically, where people record observations incidentally rather than through a recurring monitoring program.
...
Let's use the [{galah} package]() to download duck records in Melbourne from 2017-2021 to get records from years before and during COVID-19 lockdowns. 
...
We want to compare record counts in 2020-2021 to previous years, but because records added the ALA have increased each year, comparing raw numbers will be an unequal comparison and bias our results.
...
Now we'll split `ducks_weekly` into 5 `data.frame`s for each year and recursively divide each weekly record count by the total records for that year with `map2()`. At the end we'll bind the separate `data.frame`s into one again with `bind_rows()`.

```{r}
ducks_scaled <- ducks_weekly %>%
  split(.$year) %>%
  map2(.x = .,
       .y = ducks_yearly$year_records, 
       ~ .x %>%
...
To compare the "normal" number of records collected prior to COVID-19 to the numbers collected throughout 2020 and 2021, it would be nice to simplify these numbers into 2 lines: one with the the average weekly "normal" count, and one with the weekly count over 2020 and 2021. 

To create the average weekly "normal" line, let's calculate the mean proportion of records each week from 2017-2019.

To do this, we'll place our weekly proportions in separate columns using `pivot_wider()` and calculate the mean proportion of observations each week across `year_2017`, `year_2018` and `year_2019` columns.
...
We have all the numbers we need for plotting, but just need to reorganise them so they plot correctly. 

We have two separate columns containing 2020 and 2021 proportional counts, weeks 1-53 (because 2021 didn't start exactly when the week ended). To plot these counts as one continuous line from Jan 2020 to Dec 2021 (106 weeks total), we'll need to place our 2021 counts under our 2020 counts in the same column and convert the week numbers for 2021 to 54-106.
...
Because we want to plot 2020 and 2021 on the same plot, we'll use `ifelse()` to make sure our week numbers in 2021 match our week numbers in `ducks_final`.
...
lockdowns <- tibble(n_lockdown, start_date, end_date) |>
  mutate(
    ...
    week_start = ifelse(year(start_date) == 2020, 
                        week(start_date), week(start_date) + 53),
    week_end = ifelse(year(end_date) == 2020, 
                      week(end_date), week(end_date) + 53),
    )
lockdowns 
```
...
The plot above is enough to see everything we need to see from our data. You *could* stop here if you wished, but we've made a more refined visualisation with nice fonts, axis scales, axis labels and titles:
...
This post has detailed how you can use ALA data to explore relationships between record counts and events. Although we can't make any causal claims from what we see in our plot, it's a nice way to do some exploratory analysis on a lot of data!
