...
ducks |> slice_sample(n = 10)
```

We'll pull out the week and year of each date and count the total observations for each week.
...
We want to compare observations recorded in 2020-2021 to previous years, but because records added the ALA have increased each year, comparing raw numbers will be an unequal comparison and bias our results.

To avoid this, let's *scale* our record counts by the total number of *Anatidae* observations each year so that we compare proportions rather than raw numbers. 
...
Now we'll split `ducks_weekly` into 5 `data.frame`s for each year and recursively divide each weekly record count by the total records for that year with `map2_dfr()` (which sticks them back together again).

```{r}
ducks_scaled <- ducks_weekly %>%
  split(.$year) %>%
  map2_dfr(.x = .,
           .y = ducks_yearly$year_obs, 
           ~ .x %>%
             mutate(prop = .x$week_obs / .y) %>%
             select(-week_obs)
  )
...
To compare observations in years prior to COVID-19 to observations over 2020 and 2021, we want to plot 2 lines: 

  1. A baseline of average observation counts in 2017 to 2019 
...
We have two separate columns in `ducks_scaled` containing proportional counts (`year_2020` and `year_2021`). Although there are 52 weeks in a year, our counts extend from weeks 1-53 because 2020 didn't start or end *exactly* at the end of the week — 2020 ended on a Thursday and 2021 ended on a Friday. 
...
#| warning: false
ducks_scaled <- ducks_scaled |>
  rows_update(tibble(week = 1, year_2021 = sum(ducks_scaled$year_2020[53] + ducks_scaled$year_2021[1]))) |>
  rows_update(tibble(week = 53, year_2020 = NA)) # remove 2020's week 53 count

ducks_scaled |> slice_head(n = 3)
ducks_scaled |> slice_tail(n = 3)
```

To allow us to plot proportional counts from Jan 2020 to Dec 2021 (105 weeks total) as one line, we'll separate our 2021 proportional counts, revise 2021 week counts to be from 53-105, and stick them under our 2020 proportions. That'll let us plot from week 1 to week 105!
...
ducks_final <- ducks_scaled |>
  ...
  bind_rows(ducks_scaled_2021) # attach 2021 to the bottom
