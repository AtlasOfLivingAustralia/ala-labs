...
Next let's summarise how many points are in each polygon. Our `intersect` object has matched each point to a suburb, but has returned this information as a list. To summarise our counts, it's easiest to convert this information back to a `tibble`. So, we can flatten our `intersect` object from a list to a vector of integers using `flatten_int()`, save them as a tibble, group by each suburb, and summarise the count of records using `n()`.
